...
import { DEFAULT_TOOL_CALL_TIMEOUT_MS, SERVER_NAME_RE, reconnectSchema } from "./model.js";
...
export function workspaceMcpFile(projectRoot) {
 ...
 */
export async function readWorkspaceServers(projectRoot) {
    ...
    try {
        ...
        return { path, ok: true, error: null, servers: parsed.servers };
    }
    catch (error) {
        return {
            path,
 ...
 */
export function validateWorkspaceServer(server) {
    const problems = [];
    ...
    for (const [header, ref] of Object.entries(server.headerRefs ?? {})) {
        if (header.trim() === "")
            problems.push("headerRefs 中存在空 header 名");
...
/** 写回一个工作区的声明文件（原子写 + 写锁）。 */
export async function writeWorkspaceServers(projectRoot, servers) {
    const path = workspaceMcpFile(projectRoot);

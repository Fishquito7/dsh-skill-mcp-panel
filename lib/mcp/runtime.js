...
import { loadMcpClientComponent, mountWorkspaceServers } from "./mount.js";
...
export class WorkspaceMcpRuntime {
    ...
    constructor(ctx) {
        this.ctx = ctx;
        ctx.effect(() => ctx.on("agent/created", ({ agent }) => this.begin(agent)), "dsh-skill-mcp-panel: workspace mcp mount");
        ...
        try {
            ...
            for (const server of store.servers) {
                ...
                if (conflicts.has(server.serverName)) {
                    ...
                    // 而不是静默遮蔽全局配置。
                    this.log("warn", "[dsh-skill-mcp-panel] " + server.serverName + " 与全局作用域同名（cordis.patch.yml），工作区声明被忽略");
                    continue;
                }
                ...
                if (resolved.input === undefined) {
                    if (resolved.error !== undefined)
                        this.log("warn", "[dsh-skill-mcp-panel] " + resolved.error);
                    continue;
                }
                for (const ref of resolved.missing)
                    missingCredentials.push(server.serverName + ":" + ref);
            ...
            for (const outcome of outcomes) {
                if (outcome.ok)
                    mounted.push(outcome.serverName);
                else
                    this.log("warn", "[dsh-skill-mcp-panel] " + outcome.serverName + " 挂载失败：" + String(outcome.error ?? ""));
            }

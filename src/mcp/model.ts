/**
 * dsh-skill-mcp-panel —— MCP 服务器配置模型（**全局作用域**）。
 *
 * 本模块只描述写进 profile `cordis.patch.yml` 受管块的那一条 patch 行，因此不含
 * scope 字段；工作区作用域（`<工作区>/.dsh/mcp.json`）另有一套只存键名的模型，
 * 见 `workspace-store.ts`。
 *
 * env/headers 的 null 是编辑语义：string = 覆盖该 key，null = 删除该 key，
 * 不出现 = 保留旧值。
 */
import { z } from "zod";
...
/** 重连策略：全局 patch 行与工作区声明文件共用同一份 schema。 */

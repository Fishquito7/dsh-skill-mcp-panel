 ...
 *   dsh-panel skill update [--yes]           检查并更新插件（默认 --profile web；走无版本 tarball）
...
function usage() {
  console.log([
    ...
    "  dsh-panel skill update [--yes] [--profile <name>]          检查最新版本，有更新则自动安装（默认 web 配置，走无版本 tarball）",
    "",
 ...
 * 执行更新命令（透传输出），返回退出码。
 *
 * INSTALL_SPEC 是无版本号 tarball（releases/latest/download/…），它本身永远指向最新
 * release，所以这里不再拼 `#v<version>`：装了 v 版本号反而会把它钉死，下次更新只会
 * 反复装同一个 commit。
 */
function runUpdate(profile: string): Promise<number> {
  return new Promise<number>((resolve) => {
    const child = spawn("dsh", ["plugin", "--profile", profile, "add", INSTALL_SPEC], {
      stdio: "inherit",
...
export async function runSkillCli(args: string[]): Promise<number> {
  ...
  if (command === "update") {
    ...
    if (!flags.yes) {
      const ok = await confirm("是否更新到 v" + latest + "？将运行 dsh plugin --profile " + flags.profile + " add " + INSTALL_SPEC + " (y/N): ");
      if (!ok) {
        console.log("已取消");
        return 0;
      }
    }
    const exit = await runUpdate(flags.profile);

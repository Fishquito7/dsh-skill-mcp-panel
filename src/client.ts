...
const css = cssChrome + cssCards + cssAdd + cssScope + cssMigrate + cssCategory + cssTree + cssIcon + cssIconMcp + cssEffects + cssChipRefresh + cssGroups;
		...
		const c = {
			...
			scopeMeasurer: "SKV_scopeMeasurer",
...
function SkillsSection(props) {
			...
			const scopeBarRef = react.useRef(null);
			const scopeMeasurerRef = react.useRef(null);
			const [scopeOverflow, setScopeOverflow] = react.useState(false);
...
// 拖放：支持拖入 .md / .zip 文件与技能文件夹（自动识别结构）。
			...
			// 作用域栏自适应：chips 放不下当前宽度时切换为下拉框。
			// 隐藏测量副本始终渲染，避免"显示 select 后不再溢出"造成的来回抖动。
			const scopeChipSignature = scopeKeys.map((key) => key + ":" + scopeCount(key)).join("\u0000");
			react.useEffect(() => {
				const bar = scopeBarRef.current;
				const measurer = scopeMeasurerRef.current;
				if (bar === null || measurer === null) return;
				const measure = () => setScopeOverflow(measurer.scrollWidth > bar.clientWidth);
				measure();
			...
			const scopeChip = (key) => (0, react_jsx_runtime.jsxs)("button", {
				type: "button",
			...
			return (0, react_jsx_runtime.jsx)("div", {
				...
				updateBanner !== null ? (0, react_jsx_runtime.jsxs)("div", {
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						ref: scopeBarRef,
						children: scopeOverflow ? (0, react_jsx_runtime.jsx)(primitives.Menu, {
							open: scopeMenuOpen,
							onClose: () => {
								setScopeMenuOpen(false);
							},
							items: scopeKeys.map((key) => ({ id: key, label: (key === "global" ? t("scopeGlobal") : labelOf(key)) + " (" + scopeCount(key) + ")" })),
							...
							anchor: (0, react_jsx_runtime.jsx)("button", {
								...
								children: [(0, react_jsx_runtime.jsx)("span", {
									...
									children: (scopeFilter === "global" ? t("scopeGlobal") : labelOf(scopeFilter)) + " (" + scopeCount(scopeFilter) + ")"
								}), (0, react_jsx_runtime.jsx)(primitives.IconChevronDownOutline14, {
									size: 14,
									className: c.scopeSelectChevron
								})]
							})
						}) : scopeKeys.map(scopeChip)
					}),
					(0, react_jsx_runtime.jsx)("div", {
						className: c.scopeMeasurer,
						ref: scopeMeasurerRef,
						"aria-hidden": "true",
						children: scopeKeys.map(scopeChip)
					}),
